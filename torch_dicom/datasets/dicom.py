...
class DicomInput(IterableDataset):
    ...
    Args:
        ...
        ignore_zero: If True, the image is normalized to [0, 1] ignoring zeros.
    """
        ...
        for dcm in iterable:
            try:
                yield self.load_example(
                    dcm, self.img_size, self.transform, self.volume_handler, self.normalize, self.ignore_zero
                )
    ...
    def load_example(
        ...
        Returns:
            A DicomExample
        """
        example = DicomInput.load_raw_example(dcm, img_size, volume_handler, normalize, ignore_zero)
    ...
    def load_pixels(
        ...
        if normalize:
            pixels = cls.normalize_pixels(pixels, ignore_zero=ignore_zero)
        return pixels
    ...
    def normalize_pixels(
        ...
        # Some mammograms have a lot of black pixels, so we have an option to ignore them when normalizing.
        # This improves contrast.
        considered_for_normalize = pixels[pixels != 0] if ignore_zero else pixels
        if not considered_for_normalize.numel():
            considered_for_normalize = pixels

        pmin, pmax = considered_for_normalize.aminmax()
        delta = (pmax - pmin).clip(min=eps)
        pixels = (pixels.float() - pmin).div_(delta).clip_(min=0, max=1)
        return pixels
    ...
    def load_raw_example(
        ...
        Args:
            ...
            volume_handler: Volume handler to be used to load the DICOM image.
            normalize: If True, the image is normalized to [0, 1].
            ignore_zero: If True, the image is normalized to [0, 1] ignoring zero values.
        ...
        pixels = cls.load_pixels(dcm, volume_handler, normalize, ignore_zero)
...
class DicomPathInput(DicomInput, PathInput):
    ...
    def load_example(
        ...
        with pydicom.dcmread(path) as dcm:
            example = super().load_example(dcm, img_size, transform, volume_handler, normalize, ignore_zero)
...
class DicomPathDataset(PathDataset):
    ...
    def __getitem__(self, idx: int) -> DicomExample:
        ...
        return self.load_example(
            path, self.img_size, self.transform, self.volume_handler, self.normalize, self.ignore_zero
        )
    ...
    def load_example(
        ...
        return DicomPathInput.load_example(path, img_size, transform, volume_handler, normalize, ignore_zero)
